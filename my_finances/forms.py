...
class IncomeForm(IncomeOutcomeForm):
    class Meta:
        model = Income
        fields = ['value', 'date', 'type', 'repetitive', 'repetition_interval', 'repetition_time', 'repetition_end', 'comment_text']
    ...
    repetition_interval = forms.ChoiceField(choices=Income.RInterval.choices, initial=1)
    ...
    def is_valid(self):
        ...
        value = self.cleaned_data.get('value')
        if value < 10:
            self.add_error('value', "Can't be less than 10")
            is_valid = False
...
class BalanceForm(forms.ModelForm):
    class Meta:
        model = Balance
        fields = ['value', 'date', 'type', 'comment_text']
